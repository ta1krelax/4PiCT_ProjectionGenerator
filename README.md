...
(cone-beam projection)，耦合 ASTRA 重建 + SSIM 评估，自动搜索"要多少投影角度数
N 才能让重建质量落到目标 SSIM 附近"，支持批量跑一组不同的样品。
...
采样与设备、样品评估、序列生成、单次重建实验、批量实验(N搜索)），点一下弹出
一个独立小窗口调那部分参数，再点一下或关掉那个小窗口就收起；几个参数窗口
互不影响，可以同时开几个对照着调。
...
12. **单次重建实验**：跑一次"生成(可选) -> ASTRA重建 -> 算SSIM"，不做N搜索，
    用来快速验证重建算法本身、或者调重建参数（算法/迭代次数/体素分辨率）看
    效果——比如手上已经有一份之前"生成全部4π投影"导出的N=3000数据，想先看看
    这份数据重建出来怎么样，不用重新走一遍生成。
    - **输入来源**二选一：
      - "使用当前加载的模型直接生成"：用当前"锥束几何/材料/采样与设备"面板
        里的设置，加一个单独的N（不跟其它地方共用），现场生成再重建。
      - "从已导出的数据集文件夹加载"：选一个包含 `projections.npy` /
        `astra_vectors.npy` / `metadata.json` 的文件夹（"生成全部4π投影"/
        "序列生成"/"批量实验"的输出都符合这个格式），跳过重新生成直接重建。
        如果 `metadata.json` 里记的 `stl_path` 那个STL文件还在，会自动读取
        算SSIM；文件挪了/删了的话就只显示重建结果，SSIM显示"无法计算"。
    - 重建算法/迭代次数/体素分辨率/体积边界余量都单独设，不跟批量实验面板
      共用（避免改批量实验参数时不小心影响这边，或者反过来）。
    - 跑完会在面板下方直接画出重建体积的三个正交中间切片（XY/XZ/YZ），如果
      有ground truth会再画一行对比切片，方便直接用眼睛看重建质量，不用只看
      SSIM一个数字。
    - "保存结果到..."可以把这次的 `reconstruction.npy` / `ground_truth.npy` /
      `single_experiment_result.json` 存到你选的目录，跟批量实验面板输出的
      文件命名一致。
13. **批量实验(N搜索)**：这是核心功能——给一批不同的STL样品，每个样品自动
...
单次重建实验面板验证过一致性：同一个球体样品，"直接生成"模式和"加载已导出
数据集"模式（先用"生成全部4π投影"导出，再用单次重建面板加载那份导出）算出
的SSIM完全一样（0.16385020776...精确到小数点后16位），确认加载已导出数据集
时读取的旋转中心/mu/分辨率这些元数据没读错。
